...
const schema = z.object({
  ...
  consent: z.literal(true, {
    errorMap: () => ({ message: 'Please confirm your consent' }),
  }),
...
export function Appointment() {
  ...
  const filteredDoctors = selectedSpeciality
    ? doctors.filter(
        (d) =>
          d.specialitySlug ===
          specialities.find((s) => s.name === selectedSpeciality)?.slug,
      )
    : doctors;

  // Pre-fill doctor from URL ?doctor= param
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const doctorSlug = params.get('doctor');
    if (doctorSlug) {
      const doctor = doctors.find((d) => d.slug === doctorSlug);
      if (doctor) reset({ doctor: doctor.name, speciality: doctor.speciality });
    }
  }, [reset]);
            ...
            {status === 'success' ? (
              ...
              >
                ...
                <p style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>
                  We've received your appointment request. Our team will contact you shortly to
                  confirm — this is not a confirmed booking yet.
                </p>
                <button
              ...
              >
                ...
                <div className="checkbox-group">
                  ...
                  <label
                    htmlFor="consent"
                    style={{ fontSize: '0.875rem', color: 'var(--text-muted)', cursor: 'pointer', lineHeight: 1.5 }}
                  >
